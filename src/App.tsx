...
function App() {
  ...
  const scoutTrackedMatches = useMemo(
    () =>
      scoutMatches
        ...
        .filter(({ tracker }) => scoutTrackerVisibleForFilter(tracker, scoutTrackerFilter, scoutToday))
        .sort((left, right) => compareScoutTrackerEntries(left.tracker, right.tracker)),
    [scoutMatches, scoutToday, scoutTracker, scoutTrackerFilter],
  )
  ...
  const scoutTrackerFilterCounts = useMemo(
    () => ({
      ...
      due: scoutJobs.filter((job) =>
        scoutTrackerIsDueNow(scoutTrackerDueState(scoutTracker[job.id] ?? defaultScoutTrackerEntry(), scoutToday)),
      ).length,
  ...
  function downloadScoutTracker(format: 'md' | 'csv') {
            ...
            <div className="scout-panel">
              ...
              {scoutJobs.length > 0 && (
                <div className="scout-tracker-summary" aria-label="Application tracker summary">
                  ...
                  <div className="scout-tracker-actions" aria-label="Tracker export actions">
                    <button className="icon-button" onClick={() => downloadScoutTracker('md')} type="button">
                      {downloaded === 'scout-md' ? <Check size={17} /> : <Download size={17} />}
                      <span>
                        {downloaded === 'scout-md'
                          ? 'Markdown exported'
                          : downloaded === 'scout-md-error'
                            ? 'Export failed'
                            : 'Export Markdown'}
                      </span>
                    </button>
                    <button className="icon-button" onClick={() => downloadScoutTracker('csv')} type="button">
                      {downloaded === 'scout-csv' ? <Check size={17} /> : <FileText size={17} />}
                      <span>
                        {downloaded === 'scout-csv'
                          ? 'CSV exported'
                          : downloaded === 'scout-csv-error'
                            ? 'Export failed'
                            : 'Export CSV'}
                      </span>
                    </button>
                    ...
                    return (
                      <article className={`scout-match-card ${match.status}`} key={match.job.id}>
                        ...
                        <div className="scout-tracker-card" aria-label={`${match.job.title} application tracker`}>
                          ...
                          <div className={`scout-due-strip ${dueState}`}>
                            <span className={`status-light ${scoutTrackerDueLight(dueState)}`} aria-hidden="true"></span>
                            <strong>{scoutTrackerDueLabels[dueState]}</strong>
                            <span>{scoutTrackerDueDetail(tracker, dueState)}</span>
                          </div>
                          <div className="scout-tracker-grid">
                            ...
                            <label className="scout-tracker-field">
                              ...
                              <input
                                aria-label={`${match.job.title} follow-up date`}
                                onChange={(event) =>
                                  updateScoutTracker(match.job.id, {
